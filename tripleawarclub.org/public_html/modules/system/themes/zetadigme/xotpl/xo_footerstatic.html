	...
	<div id="controls">
		<div>
			<ul>
				...
				<li class="chat text">Powered by <a href="http://sourceforge.net/projects/xoops/" title="XOOPS Project HomePage"><{$xoops_version}></a></li>
				<li class="chat separate">&nbsp;</li>			
				...
				<{xoInboxCount assign=pmcount}>
					<li class="chat">
						<a href="<{xoAppUrl viewpmsg.php}>" title="<{$smarty.const.THEME_MESSAGE}> (<{$pmcount}>)">
							<{if $pmcount}>
								<!-- if I have messages -->
								<img src="<{xoImgUrl img/mail_warning.png}>" alt="(<{$pmcount}>) <{$smarty.const.THEME_NOTREAD}>"/>(<span class="shadow" style="color:#ff0000; font-weight: bold;"><{$pmcount}></span>)
							<{else}>
