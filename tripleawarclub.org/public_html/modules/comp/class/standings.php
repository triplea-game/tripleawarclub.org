...
class CompStandingsHandler {
	 ...
	 * @var $provisional show provisional players 1, default 0 is no
	 * @return mixed array of all players from the competition with given id sorted by rating
	 */
	function getStandings($comp_id,$provisional=0) {
		
		// Create a Cache_Lite object
		$cache_handler = xoops_getmodulehandler('cachelite');
		$cacheoptions = array('cacheDir' => XOOPS_ROOT_PATH.'/modules/comp/class/Lite/',  'lifeTime' => 3600);
		...
		// Check database for information
 			...
 			$sql = "SELECT $comp_user_table.*, $xoops_user_table.uname, $xoops_user_table.email, " .
 					...
 					"WHERE $comp_user_table.xoops_user_id = $xoops_user_table.uid " .
 						...
 						"AND $comp_user_table.status < 2 " .
						"AND $comp_user_table.provisional = ".$provisional.
			...
			while( $row = $this->db->fetchArray($result) ){
				...
				$row['options'] = $player_handler->getPlayerOptions(
					$row['option_rules'], $row['option_luck'], $row['option_mode'],$row['nos'],$row['map'],$row['option_mode'],$comp_id);

				// Save the player's rank
				if($i==0 && $provisional==0){ // top non-provisional player 
