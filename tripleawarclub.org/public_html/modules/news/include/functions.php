<?php
// $Id: functions.php 10043 2012-08-08 22:36:24Z beckmi $
//  ------------------------------------------------------------------------ //
...
{
	global $xoopsDB;
	$query = 'SELECT rating FROM '.$xoopsDB->prefix('mod_news_stories_votedata').' WHERE storyid = '.$storyid;
	...
	$sql = sprintf("UPDATE %s SET rating = %u, votes = %u WHERE storyid = %u", $xoopsDB->prefix('mod_news_stories'), $finalrating, $votesDB, $storyid);
	$xoopsDB->queryF($sql);
}
