...
function xoops_module_update_news() {
   ...
   global $xoopsDB;
    ...
    {
        $sql=sprintf('ALTER TABLE ' . $xoopsDB->prefix('stories_files') .  ' RENAME ' .$xoopsDB->prefix('mod_news_stories_files'));
    ...
    {
        $sql=sprintf('ALTER TABLE ' . $xoopsDB->prefix('stories') . ' RENAME ' .$xoopsDB->prefix('mod_news_stories'));
        	$result=$xoopsDB->queryF($sql);
        	if (!$result) {
            	echo '<br />' .  _AM_NEWS_UPGRADEFAILED.' '._AM_NEWS_UPGRADEFAILED2;
            	$errors++;
    ...
    {
        $sql=sprintf('ALTER TABLE ' . $xoopsDB->prefix('topics') . ' RENAME ' .$xoopsDB->prefix('mod_news_topics'));
        	$result=$xoopsDB->queryF($sql);
        	if (!$result) {
            	echo '<br />' .  _AM_NEWS_UPGRADEFAILED.' '._AM_NEWS_UPGRADEFAILED2;
            	$errors++;
