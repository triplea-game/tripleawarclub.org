<?php
// $Id: ratenews.php 10043 2012-08-08 22:36:24Z beckmi $
//  ------------------------------------------------------------------------ //
...
if(!empty($_POST['submit'])) {			// The form was submited
	...
	if ($ratinguser != 0) {
		$result=$xoopsDB->query('SELECT uid FROM '.$xoopsDB->prefix('mod_news_stories')." WHERE storyid=$storyid");
		...
		$result=$xoopsDB->query('SELECT ratinguser FROM '.$xoopsDB->prefix('mod_news_stories_votedata')." WHERE storyid=$storyid");
		while(list($ratinguserDB)=$xoopsDB->fetchRow($result)) {
			if ($ratinguserDB==$ratinguser) {
		...
		$result=$xoopsDB->query('SELECT COUNT(*) FROM '.$xoopsDB->prefix('mod_news_stories_votedata')." WHERE storyid=$storyid AND ratinguser=0 AND ratinghostname = '$ip'  AND ratingtimestamp > $yesterday");
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix('mod_news_stories_votedata').'_ratingid_seq');
	$datetime = time();
	$sql = sprintf("INSERT INTO %s (ratingid, storyid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $xoopsDB->prefix("mod_news_stories_votedata"), $newid, $storyid, $ratinguser, $rating, $ip, $datetime);
